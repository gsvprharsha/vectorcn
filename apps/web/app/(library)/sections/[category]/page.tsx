...
const categories = [
  { slug: "hero-section", name: "Hero" },
  { slug: "notification-bars", name: "Notification" },
];
...
export default async function Page({ params }: PageProps) {
  ...
  let CategoryComponent: React.ComponentType;

  try {
    const mod = await import(`../../_components/sections/${slug}`);
    CategoryComponent = mod.default;
  } catch (err) {
    notFound();
  }
        ...
        <section className="mt-14 bg-background flex flex-col min-h-screen w-full max-w-6xl border-x border-dashed">
          ...
          <div className="mt-5 py-4">
            ...
            <div className="text-center">
              <CategoryNav categories={categories} selectedSlug={slug} />
            </div>
